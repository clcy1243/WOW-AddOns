...
function Outfitter._PawnScaleStat:GetItemScore(pItem)
	Outfitter:DebugMessage("PawnScaleStat:GetItemScore: "..tostring(pItem and pItem.Link))
	...
	-- Leave if the item is too high-level
	if vStats.minLevel > UnitLevel("player") then
		Outfitter:DebugMessage("PawnScaleStat:GetItemScore: returning nothing because item is too low level")
		return
	end
	...
	if not vItemData then
		Outfitter:DebugMessage("PawnScaleStat:GetItemScore: returning nothing because there's no item data")
		return
	end
	...
	for _, vEntry in pairs(vItemData.Values) do
		...
		if vScaleName == self.Name then
			Outfitter:DebugMessage("PawnScaleStat:GetItemScore: returning "..tostring(vValue))
			return vValue
		end
	end
	Outfitter:DebugMessage("PawnScaleStat:GetItemScore: returning nothing because it wasn't found")
end
