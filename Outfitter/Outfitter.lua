...
function Outfitter:CreateEmptySpecialOccasionOutfit(pScriptID, pName, pAllowDuplicates)
	vOutfit = self:GetOutfitByScriptID(pScriptID)
...
function Outfitter:InitializeTalentTreeOutfits()
	...
	for treeIndex = 1, 4 do
		local _, treeName = GetSpecializationInfo(treeIndex)
		...
		-- Create the outfit
		local outfit = self:CreateEmptySpecialOccasionOutfit("TalentTree", playerClass..": "..treeName, true)

		-- Configure the script
		outfit.ScriptSettings = {
			Tree1 = treeIndex == 1,
			Tree2 = treeIndex == 2,
			Tree3 = treeIndex == 3,
			Tree4 = treeIndex == 4,
...
function Outfitter._NameOutfitDialog:Update(pCheckForStatOutfit)
	...
	and self.GenerateOutfitCheckButton:GetChecked() then
		...
		Outfitter:DebugTable(vStat, "stat")
