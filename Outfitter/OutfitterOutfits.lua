...
function Outfitter._OutfitMethodsEM:SaveEquipmentSet(pName, pIconTexture)
	if Outfitter.Debug.NewItems then
		Outfitter:DebugMessage("%s:SaveEquipmentSet(%s)", pName, tostring(pIconTexture))
	...
	local vTexture
	if type(pIconTexture) == "string" then
		_, _, vTexture = pIconTexture:find("([^\\]*)$")
		vTexture = vTexture:upper()
	else
		vTexture = pIconTexture
	end
	SaveEquipmentSet(pName, vTexture)
