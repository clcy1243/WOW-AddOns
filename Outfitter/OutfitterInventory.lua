...
function Outfitter:GetItemInfoFromLink(pItemLink)
	...
	local _, _,
	      vItemQuality,
	      vItemLevel,
	      vItemMinLevel = GetItemInfo(pItemLink)
	
	vItemInfo.Name = vItemName
	vItemInfo.Link = pItemLink
	vItemInfo.Quality = vItemQuality
	vItemInfo.Level = vItemLevel
	vItemInfo.MinLevel = vItemMinLevel

	-- fill in the rest from vItemCodes
	
	vItemInfo.EnchantCode = vItemCodes[2] or 0
	vItemInfo.JewelCode1 = vItemCodes[3] or 0
	vItemInfo.JewelCode2 = vItemCodes[4] or 0
	vItemInfo.JewelCode3 = vItemCodes[5] or 0
	vItemInfo.JewelCode4 = vItemCodes[6] or 0
	vItemInfo.SubCode = vItemCodes[7] or 0
	vItemInfo.UniqueID = vItemCodes[8] or 0
	-- vItemInfo.LinkLevel = vItemCodes[9]		-- LinkLevel is the level of the player who generated the link, which isn't interesting to us
	-- vItemInfo.SpecializationID = vItemCodes[10]	-- DON'T capture this
	vItemInfo.UpgradeTypeID = vItemCodes[11] or 0
	vItemInfo.InstanceDifficultyID = vItemCodes[12] or 0

	local vNumBonusIDsString = vItemCodes[13]
	local vIndex = 14

	local vNumBonusIDs = tonumber(vNumBonusIDsString)
	local vBonusID1, vBonusID2
	if vNumBonusIDs > 0 then
		vBonusID1 = vItemCodes[vIndex]
		vIndex = vIndex + 1
	end
	if vNumBonusIDs > 1 then
		vBonusID2 = vItemCodes[vIndex]
		vIndex = vIndex + 1
	end

	vItemInfo.BonusIDs = (vNumBonusIDsString ~= 0 and vNumBonusIDsString or "")..":"..(vBonusID1 ~= 0 and vBonusID1 or "")..":"..(vBonusID2 ~= 0 and vBonusID2 or "")
	vItemInfo.UpgradeID = vItemCodes[vIndex] or 0
