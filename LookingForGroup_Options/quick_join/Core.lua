				...
				if q1 ~= nil then
					...
					if queue_data ~= nil then
						...
						if lfgListID ~= nil then
							if filter_search_results(lfgListID,filters) then
	...
	args =
	{
		...
		enable =
		{
			...
			set = function(info,val)
				LookingForGroup.db.profile.enable_qj = val
				LookingForGroup_Options:LFG_LIST_APPLICANT_UPDATED()
				LookingForGroup_Options:OnEnable()
			end
		},
		s =
		{
