...
local LookingForGroup_GetModule = LookingForGroup.GetModule
local _G = _G
...
local function applicant_info(appid)
	...
	comment = string_match(comment,'^(.*)%((^1^.+^^)%)$')
	...
	return table_concat(concat_tb),table_concat(concat_tb_tooltip)
end
...
AceGUI:RegisterWidgetType("LookingForGroup_Options_Applicant_Multiselect", function()
	...
	control.SetItemValue = function(self,key)
		...
		local label = applicant_info(val)
