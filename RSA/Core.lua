...
--------------------------------------
RSA = LibStub("AceAddon-3.0"):NewAddon("RSA", "AceConsole-3.0", "LibSink-2.0", "AceEvent-3.0")
			...
			Spells = {
				Misdirection = {
					Messages = {
						...
						End = "[LINK] on [TARGET] ended! Granted [AMOUNT] additional threat to you!",
					},
					CustomChannel = {
						Enabled = false,
						Channel = "",
		...
		Shaman = {
			...
			Spells = {
				...
				AncestralProtection = {
					Messages = {
						Start = "[LINK] placed!",
						Success = "[LINK] resurrected [TARGET]!",
						End = "[LINK] ended!",
					},
					CustomChannel = {
						Enabled = false,
		...
		Warlock = {
			...
			Spells = {
				...
				Shadowfury = {
					Messages = {
						Cast = "Casting [LINK]!",
						Applied = "[LINK] Activated!",
						Finish = "[LINK] finished!",
					},
					CustomChannel = {
						Enabled = false,
						Channel = "",
...
function RSA:OnInitialize() -- Do all this when the addon loads.
	...
	if tonumber(GetAddOnMetadata("RSA", "Version")) > tonumber(self.db.global.version) then -- Print when addon is updated.
		print("|cFFFF75B3Raeli's Spell Announcer|r updated to version " .. "|cff00FF00" .. GetAddOnMetadata("RSA", "Version") .. " " .. (GetAddOnMetadata("RSA", "X-Curse-Packaged-Version") or "") .. "|r.")		
	end
	...
	elseif pClass == "DEMONHUNTER" then
...
function RSA.Print_LibSink(message)
 -- Sends a message set by LibSink-2.0. Could just use :Pour on the messages themselves, 
 -- but this is here to save work later if I need to add any additional checks.
	RSA:Pour("|cFFFF75B3RSA:|r " .. message, 1, 1, 1)
end
function RSA.BossBars(message, timeInSeconds, iconPath)
	-- Create a DBM or BigWigs bar if the user has the addons enabled.
	--if not BigWigs:IsEnabled() then BigWigs:Enable() end
	--RSA = {}; LibStub("AceEvent-3.0"):Embed(RSA)
	
	--BigWigsLoader:SendMessage("BigWigs_StartBar", RSA, "", message, timeInSeconds, iconPath)
	--SendMessage("BigWigs_StartBar", yourAddonObject, optionKey, "Test bar", timeInSeconds, iconPath)
	
	
	--DBM:CreatePizzaTimer(timeInSeconds, message, false, _, _, true)
